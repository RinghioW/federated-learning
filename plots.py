...
def plot_results(results_dir, num_users, latency_histories, data_imbalance_histories, obj_functions, losses, accuracies):
    ...
    for user_idx in range(num_users):
        ...
        plt.xlabel("Epoch")
        plt.ylabel("Latency")
        plt.savefig(results_dir + f"latency_history_user_{user_idx+1}.png")
        plt.close()
        ...
        plt.savefig(results_dir + f"data_imbalance_history_user_{user_idx+1}.png")
        plt.close()
        ...
        plt.savefig(results_dir + f"objective_function_user_{user_idx+1}.png")
        plt.close()
